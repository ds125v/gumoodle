 ...
 *
 * @package gu_mathskills
 * @subpackage questiontypes
 */
class question_gu_mathskills_qtype extends default_questiontype {
	...
	function import_from_xml( $data, $question, $format, $extra=null ) {
		$qtype = $data['@']['type'];
		if($qtype != 'gu_mathskills')
			return false;
			
        $qo = $format->import_headers( $data );
        // 'header' parts particular to multichoice
        $qo->qtype = 'gu_mathskills';
   ...
   function get_question_options(&$question) {
        ...
        if (!$question->options = get_record('question_gu_mathskills', 'question',
         $question->id)) {
            notify('Error: Missing question options for gu_mathskills question'.$question->id.'!');
            return false;
        }

        if (!$question->options->answers = get_records_select('question_answers', 'id IN ('.$question->options->answers.')', 'id')) {
           notify('Error: Missing question answers for gu_mathskills question'.$question->id.'!');
           return false;
        }
    ...
    function save_question_options($question) {
        ...
        $update = true;
        $options = get_record("question_gu_mathskills", "question", $question->id);
        if (!$options) {
            $update = false;
        ...
        if ($update) {
            if (!update_record("question_gu_mathskills", $options)) {
                $result->error = "Could not update quiz gu_mathskills options! (id=$options->id)";
                return $result;
            }
        } else {
            if (!insert_record("question_gu_mathskills", $options)) {
                $result->error = "Could not insert quiz gu_mathskills options!";
                return $result;
            }
        }
    ...
    */
    function delete_question($questionid) {
        delete_records("question_gu_mathskills", "question", $questionid);
        return true;
    }

    function create_session_and_responses(&$question, &$state, $cmoptions, $attempt) {
//# This should be where the random variations are set up, just need to
// remove an option
        ...
        //NSFB - new code for maths skills - remove 1 option. (28/04/10)
        $remove = rand(0,sizeof($answerids)-1);
        $state->options->remove = $remove;
        array_splice($answerids, $remove,1);
        //NSFB - end
    ...
    function print_question_formulation_and_controls(&$question, &$state, $cmoptions, $options) {
        global $CFG;
        //echo "<pre>State is ".print_r($state,1).'</pre>';
   ...
   //# This is where the new code will happen to add the 'None of the Above' I think

        // Print each answer in a separate row
        $highestdisplayedfraction = -1;
        ...
        foreach ($state->options->order as $key => $aid) {
            ...
            $highestdisplayedfraction = ($highestdisplayedfraction > $answer->fraction) ? $highestdisplayedfraction : $answer->fraction;
 ...
 //# NSFB code for adding a 'none of the above' option.
        $checked = '';
        $chosen = false;
 		$a->id = 'none';
        $a->feedbackimg = '';
        if($highestdisplayedfraction <= 0) {
           	$fraction = 1;
        }
        else {
        	$fraction = 0;
        }
        if (isset($state->responses['']) and 'none' == $state->responses['']) {
            $checked = 'checked="checked"';
            $chosen = true;
        }
        $a->control = "<input $readonly id=\"$a->id\" $name $checked $type value=\"none\" />";
        ...
        include("$CFG->dirroot/question/type/gu_mathskills/display.html");
    }
    ...
    function grade_responses(&$question, &$state, $cmoptions) {
        ...
        if($question->options->single) {
            ...
            if($response == 'none') {
		        $highestdisplayedfraction = -1;
		        foreach ($state->options->order as $key => $aid) {
        			$answer = &$question->options->answers[$aid];
		            $highestdisplayedfraction = ($highestdisplayedfraction > $answer->fraction) ? $highestdisplayedfraction : $answer->fraction;
                }
		        if($highestdisplayedfraction <= 0){
		        	$state->raw_grade = 1;
...
/// BACKUP FUNCTIONS ////////////////////////////
    ...
    function backup($bf,$preferences,$question,$level=6) {
        ...
        $gu_mathskillss = get_records("question_gu_mathskills","question",$question,"id");
        //If there are gu_mathskillss
        if ($gu_mathskillss) {
            //Iterate over each gu_mathskills
            foreach ($gu_mathskillss as $gu_mathskills) {
                $status = fwrite ($bf,start_tag("GU_MATHSKILLS",$level,true));
                //Print gu_mathskills contents
                fwrite ($bf,full_tag("LAYOUT",$level+1,false,$gu_mathskills->layout));
                fwrite ($bf,full_tag("ANSWERS",$level+1,false,$gu_mathskills->answers));
                fwrite ($bf,full_tag("SINGLE",$level+1,false,$gu_mathskills->single));
                fwrite ($bf,full_tag("SHUFFLEANSWERS",$level+1,false,$gu_mathskills->shuffleanswers));
                fwrite ($bf,full_tag("CORRECTFEEDBACK",$level+1,false,$gu_mathskills->correctfeedback));
                fwrite ($bf,full_tag("PARTIALLYCORRECTFEEDBACK",$level+1,false,$gu_mathskills->partiallycorrectfeedback));
                fwrite ($bf,full_tag("INCORRECTFEEDBACK",$level+1,false,$gu_mathskills->incorrectfeedback));
                fwrite ($bf,full_tag("ANSWERNUMBERING",$level+1,false,$gu_mathskills->answernumbering));
                $status = fwrite ($bf,end_tag("GU_MATHSKILLS",$level,true));
            }
...
/// RESTORE FUNCTIONS /////////////////
    ...
    function restore($old_question_id,$new_question_id,$info,$restore) {
        ...
        //Get the gu_mathskillss array
        $gu_mathskillss = $info['#']['GU_MATHSKILLS'];

        //Iterate over gu_mathskillss
        for($i = 0; $i < sizeof($gu_mathskillss); $i++) {
            $mul_info = $gu_mathskillss[$i];

            //Now, build the question_gu_mathskills record structure
            $gu_mathskills = new stdClass;
            $gu_mathskills->question = $new_question_id;
            $gu_mathskills->layout = backup_todb($mul_info['#']['LAYOUT']['0']['#']);
            $gu_mathskills->answers = backup_todb($mul_info['#']['ANSWERS']['0']['#']);
            $gu_mathskills->single = backup_todb($mul_info['#']['SINGLE']['0']['#']);
            $gu_mathskills->shuffleanswers = isset($mul_info['#']['SHUFFLEANSWERS']['0']['#'])?backup_todb($mul_info['#']['SHUFFLEANSWERS']['0']['#']):'';
            if (array_key_exists("CORRECTFEEDBACK", $mul_info['#'])) {
                $gu_mathskills->correctfeedback = backup_todb($mul_info['#']['CORRECTFEEDBACK']['0']['#']);
            } else {
                $gu_mathskills->correctfeedback = '';
            }
            if (array_key_exists("PARTIALLYCORRECTFEEDBACK", $mul_info['#'])) {
                $gu_mathskills->partiallycorrectfeedback = backup_todb($mul_info['#']['PARTIALLYCORRECTFEEDBACK']['0']['#']);
            } else {
                $gu_mathskills->partiallycorrectfeedback = '';
            }
            if (array_key_exists("INCORRECTFEEDBACK", $mul_info['#'])) {
                $gu_mathskills->incorrectfeedback = backup_todb($mul_info['#']['INCORRECTFEEDBACK']['0']['#']);
            } else {
                $gu_mathskills->incorrectfeedback = '';
            }
            if (array_key_exists("ANSWERNUMBERING", $mul_info['#'])) {
                $gu_mathskills->answernumbering = backup_todb($mul_info['#']['ANSWERNUMBERING']['0']['#']);
            } else {
                $gu_mathskills->answernumbering = 'abc';
            }
            ...
            $answers_field = "";
            $in_first = true;
            $tok = strtok($gu_mathskills->answers,",");
            while ($tok) {
            ...
            $gu_mathskills->answers = $answers_field;

            //The structure is equal to the db, so insert the question_shortanswer
            $newid = insert_record ("question_gu_mathskills",$gu_mathskills);
        ...
        if ($order) {
            foreach ($order as $key => $oldansid) {
                ...
                if ($answer) {
                    ...
                    echo 'Could not recode gu_mathskills answer id '.$oldansid.' for state '.$state->oldid.'<br />';
                }
            }
        }
        if ($responses) {
            foreach ($responses as $key => $oldansid) {
                ...
                if ($answer) {
                    ...
                    echo 'Could not recode gu_mathskills response answer id '.$oldansid.' for state '.$state->oldid.'<br />';
                }
            }
        }
    ...
    function decode_content_links_caller($questionids, $restore, &$i) {
        ...
        // Decode links in the question_gu_mathskills table.
        if ($gu_mathskillss = get_records_list('question_gu_mathskills', 'question',
                implode(',',  $questionids), '', 'id, correctfeedback, partiallycorrectfeedback, incorrectfeedback')) {

            foreach ($gu_mathskillss as $gu_mathskills) {
                $correctfeedback = restore_decode_content_links_worker($gu_mathskills->correctfeedback, $restore);
                $partiallycorrectfeedback = restore_decode_content_links_worker($gu_mathskills->partiallycorrectfeedback, $restore);
                $incorrectfeedback = restore_decode_content_links_worker($gu_mathskills->incorrectfeedback, $restore);
                if ($correctfeedback != $gu_mathskills->correctfeedback ||
                        $partiallycorrectfeedback != $gu_mathskills->partiallycorrectfeedback ||
                        $incorrectfeedback != $gu_mathskills->incorrectfeedback) {
                    ...
                    if (!update_record('question_gu_mathskills', $gu_mathskills)) {
                        $status = false;
                    }
                }
     ...
     *      should be a lang string answernumberingxxx in teh qtype_gu_mathskills
    ...
    function replace_file_links($question, $fromcourseid, $tocourseid, $url, $destination){
        ...
        if ($optionschanged){
            if (!update_record('question_gu_mathskills', addslashes_recursive($newoptions))) {
                error('Couldn\'t update \'question_gu_mathskills\' record '.$newoptions->id);
            }
        }
        $answerchanged = false;
        foreach ($question->options->answers as $answer) {
...
question_register_questiontype(new question_gu_mathskills_qtype());
?>
