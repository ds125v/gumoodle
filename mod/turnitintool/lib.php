 ...
 */
define('TII_LATENCY_SLEEP',0);
/**
 * API Error: Start Error Code
 */
define('API_ERROR_START',100);
/**
 * API Error: Database Error inserting unique ID into the database
 */
define('DB_UNIQUEID_ERROR',218);
/**
 * API Error: Creating/Updating/Deleting assignment failed in fid 4
 */
define('ASSIGNMENT_UPDATE_ERROR',411);
/**
 * API Error: The assignment you are trying to access does not exist in Turnitin for this class
 */
define('ASSIGNMENT_NOTEXIST_ERROR',206);
/**
 ...
 */
define('ASSIGNMENT_WRONGCLASS_ERROR',228);
/**
 * API SRC Value: The src value defines the integration namespace area in the Turnitin 
 * integrations database tables
 */
define('TURNITIN_APISRC',12);
...
function turnitintool_add_instance($turnitintool) {
    ...
    $turnitinuser=turnitintool_usersetup($USER,get_string('userprocess','turnitintool'),$tii,$loaderbar); // PROC 1
    ...
    for ($i=1;$i<=$turnitintool->numparts;$i++) {
        ...
        $tiipost->max_points=$turnitintool->grade;
        ...
        $part->maxmarks=$turnitintool->grade;
        $part->deleted=0;
        if (!$insert=turnitintool_insert_record('turnitintool_parts',$part,false)) {
            turnitintool_delete_records('turnitintool','id',$insertid);
            turnitintool_print_error('partdberror','turnitintool','',$i,__FILE__,__LINE__);
        }
...
function turnitintool_delete_instance($id) {
    ...
    foreach ($parts as $part) {
        ...
        // Get the Turnitin UIDs [[[[
        $tiipost->cid=turnitintool_getCID($course->id);
        $tiipost->assignid=turnitintool_getAID($part->id);
        // ]]]]
    
        // Do the Assignment Deletion using the Turnitin API [[[[
        $tiipost->ctl=turnitintool_getCTL($course->id);
        $tiipost->name=$turnitintool->name.' - '.turnitintool_partnamefromnum($part->id).' (Moodle '.$tiipost->assignid.')';

        $tii->deleteAssignment($tiipost,get_string('assignmentdeleteprocess','turnitintool',$proc));

        if ($tii->getRerror() AND $tii->getRcode()!=ASSIGNMENT_UPDATE_ERROR AND $tii->getRcode()!=ASSIGNMENT_NOTEXIST_ERROR AND $tii->getRcode()!=ASSIGNMENT_WRONGCLASS_ERROR) {
            $reason=($tii->getAPIunavailable()) ? get_string('apiunavailable','turnitintool') : $tii->getRmessage();
            turnitintool_print_error('<strong>'.get_string('deleteerror','turnitintool').'</strong><br />RCODE: ('.$tii->getRcode().') '.$reason);
            exit();
        } else {
...
function turnitintool_refresh_events($courseid=0) {
    ...
    foreach ($turnitintools as $turnitintool) {
        ...
        foreach ($parts as $part) {
            ...
            if ($events = turnitintool_get_record('event', 'modulename','turnitintool','instance',$turnitintool->id,'name',$turnitintool->name.' - '.$part->name)) {
                $event->id = $events->id;
                update_event($event);
            } else {
                $event->courseid    = $turnitintool->course;
                ...
                $visibles = turnitintool_get_record('course_modules','module',$moduleid,'instance',$turnitintool->id);
                $visible=current($visibles);
                $event->visible = $visible->visible;
                add_event($event);
            }    
        }
        $result=true;
...
function turnitintool_print_overview($courses, &$htmlarray) {
    ...
    foreach ($turnitintools as $key => $turnitintool) {
        ...
        $partsarray=array();
        foreach ($parts as $part) {
            $earlydate = ($part->dtstart>$earlydate) ? $part->dtstart : $earlydate;
        ...
        if (($turnitintool->allowlate AND $earlydate<=$now) OR ($earlydate<=$now AND $latedate>=$now)) { // Turnitin Assignment Is Active for this user
...
function turnitintool_getUID($owner) {
    if (!$turnitintool_user = turnitintool_get_record("turnitintool_users", "userid", $owner->id)) {
        return NULL;
    } else {
 ...
 *
 * @deprecated This function should be removed as the user FID1 call is now required to create a session
 *          we need to refactor / remove this function as some of its functionality is now redundant / unnecessary
...
function turnitintool_usersetup($userdata,$status='',&$tii,&$loaderbar) {
    ...
    if (!$turnitintool_user = turnitintool_get_record("turnitintool_users", "userid", $userdata->id)) {
        ...
        $post->idsync=1;
        if ($tii->utp==1 AND !$CFG->turnitin_studentemail) {
            $post->dis=1;
        } else {
            $post->dis=0;
        }
        ...
        if ($tii->getRerror()) {
            if ($tii->getAPIunavailable()) {
                ...
                turnitintool_print_error($tii->getRmessage(),NULL,'',NULL,__FILE__,__LINE__);
            }
            exit();
        }
...
function turnitintool_classsetup($course,$owner,$status='',&$tii,&$loaderbar) {
    ...
    if (!$classexists) {
        ...
        $uniquestring=strtoupper(uniqid());
        $turnitin_ctl=$course->fullname." (".$uniquestring.")";
...
function turnitintool_draw_menu($cm,$do) {
    ...
    if (has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id))) {
        ...
        $tabs[] = new tabobject('submissions', $CFG->wwwroot.'/mod/turnitintool/view.php'.'?id='.$cm->id.'&do=submissions',
                                get_string('submitpaper','turnitintool'), get_string('mysubmissions','turnitintool'), false);
...
function turnitintool_update_partnames($cm,$turnitintool,$post) {
    ...
    if (has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id))) {
        $notice['message']='';
        $error=false;
        $dtstart=strtotime($post["year_dtstart"].'-'.$post["month_dtstart"].'-'.$post["day_dtstart"].' '.$post["hour_dtstart"].':'.$post["min_dtstart"].':00');
        $dtdue=strtotime($post["year_dtdue"].'-'.$post["month_dtdue"].'-'.$post["day_dtdue"].' '.$post["hour_dtdue"].':'.$post["min_dtdue"].':00');
        $dtpost=strtotime($post["year_dtpost"].'-'.$post["month_dtpost"].'-'.$post["day_dtpost"].' '.$post["hour_dtpost"].':'.$post["min_dtpost"].':00');
        if ($dtstart>=$dtdue) {
        ...
        if (!$error) {
            ...
            $post['partname']=str_replace("'","&#39;",$post['partname']); // FIX SINGLE QUOTES
            $post['partname']=str_replace('"',"&#34;",$post['partname']); // FIX DOUBLE QUOTES
...
function turnitintool_delete_part($cm,$turnitintool,$partid) {
    ...
    if ($turnitintool->numparts==1) {
        ...
        // Get the Turnitin UIDs [[[[
        $tiipost->cid=turnitintool_getCID($turnitintool->course);
        $tiipost->assignid=turnitintool_getAID($partid);
        // ]]]]

        // Do the Assignment Part Deletion using the Turnitin API [[[[
        $tiipost->ctl=turnitintool_getCTL($turnitintool->course);
        $tiipost->name=$turnitintool->name.' - '.turnitintool_partnamefromnum($partid).' (Moodle '.$tiipost->assignid.')';
        
        $tii->deleteAssignment($tiipost,get_string('assignmentdeleteprocess','turnitintool',''));
        
        if ($tii->getRerror() AND $tii->getRcode()!=ASSIGNMENT_UPDATE_ERROR AND $tii->getRcode()!=ASSIGNMENT_NOTEXIST_ERROR) {
        // ADDED CHECK TO ALLOW DELETION OF THE ASSIGNMENT WHEN THE ASSIGNMENT HAS ALREADY BEEN DELETED ON THE TURNITIN SIDE
            $reason=($tii->getAPIunavailable()) ? get_string('apiunavailable','turnitintool') : $tii->getRmessage();
            turnitintool_print_error('<strong>'.get_string('deleteerror','turnitintool').'</strong><br />RCODE: ('.$tii->getRcode().') '.$reason);
            exit();
        } else {
...
function turnitintool_introduction($cm,$turnitintool,$notice='') {
    ...
    unset($cells);
    $cells=array();
    $cells[]=get_string('turnitintoolname', 'turnitintool');
    $cells[]=$turnitintool->name;
    $table->data[]=$cells;
    
    if ($turnitintool->defaultdtstart < time() OR has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id))) {
    ...
    unset($cells);
    $cells=array();
    $cells[]=get_string('turnitintoolintro', 'turnitintool');
    $cells[]=$intro;
    $table->data[]=$cells;
    
    unset($cells);
    $cells[]=get_string('turnitintoolowner','turnitintool').turnitintool_help_icon('turnitintoolowner', get_string('turnitintoolowner','turnitintool'),
                        'turnitintool',true,false,'',true);
    $owner=turnitintool_get_owner($turnitintool->course);
    $cells[]='<a href="'.$CFG->wwwroot.'/user/view.php?id='.$owner->id.'&course='.$turnitintool->course.'">'.$owner->firstname.
                ' '.$owner->lastname.'</a> '.turnitintool_ownerselect($cm,$turnitintool);
    $table->data[]=$cells;
    ...
    $table->cellpadding='4px';
    $table->cellspacing='0px';
    $table->size=array(NULL,'15%','15%','15%','8%','3%','3%');
    $table->head=array(get_string('partname','turnitintool'),
    ...
    foreach($parts as $part) {
        ...
        if (has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id))) { // DO TUTOR VIEW
            if (isset($notice['error']) AND $notice['error']==$part->id) {
                $class=' class="error"';
                $part=turnitintool_array_to_object($notice['post']);
                $part->dtstart=strtotime($notice['post']["year_dtstart"].'-'.$notice['post']["month_dtstart"].'-'.$notice['post']["day_dtstart"].' '.$notice['post']["hour_dtstart"].':'.$notice['post']["min_dtstart"].':00');
                $part->dtdue=strtotime($notice['post']["year_dtdue"].'-'.$notice['post']["month_dtdue"].'-'.$notice['post']["day_dtdue"].' '.$notice['post']["hour_dtdue"].':'.$notice['post']["min_dtdue"].':00');
                $part->dtpost=strtotime($notice['post']["year_dtpost"].'-'.$notice['post']["month_dtpost"].'-'.$notice['post']["day_dtpost"].' '.$notice['post']["hour_dtpost"].':'.$notice['post']["min_dtpost"].':00');
                $part->id=$notice['post']['submitted'];
                $editpart='
                <script language="javascript">
                    turnitintool_editpart(parts,'.$part->id.');
                </script>';
            } else {
                $class='';
            }
            $cells=array();
            $cells[0]='<form action="'.$CFG->wwwroot.'/mod/turnitintool/view.php'.'?id='.$cm->id.'&do=intro" method="POST"><input type="hidden" name="submitted" value="'.$part->id.'" />';
            $cells[0].='<div id="partname_'.$part->id.'"'.$class.'><input name="partname" value="'.$part->partname.'" type="text" /></div>';
            $cells[0].='<span id="partnametext_'.$part->id.'"></span>';
            
            $date = array('h'=>date('H',$part->dtstart),
                          'min'=>date('i',$part->dtstart),
                          'd'=>date('j',$part->dtstart),
                          'm'=>date('n',$part->dtstart),
                          'Y'=>date('Y',$part->dtstart));
            $cells[1]='<div id="dtstart_'.$part->id.'"'.$class.'>'.turnitintool_dateselect('dtstart',$date).'</div>';
            $cells[1].='<span id="dtstarttext_'.$part->id.'"></span>';
            
            $date = array('h'=>date('H',$part->dtdue),
                          'min'=>date('i',$part->dtdue),
                          'd'=>date('j',$part->dtdue),
                          'm'=>date('n',$part->dtdue),
                          'Y'=>date('Y',$part->dtdue));
            $cells[2]='<div id="dtdue_'.$part->id.'"'.$class.'>'.turnitintool_dateselect('dtdue',$date).'</div>';
            $cells[2].='<span id="dtduetext_'.$part->id.'"></span>';
            
            $date = array('h'=>date('H',$part->dtpost),
                          'min'=>date('i',$part->dtpost),
                          'd'=>date('j',$part->dtpost),
                          'm'=>date('n',$part->dtpost),
                          'Y'=>date('Y',$part->dtpost));
            $cells[3]='<div id="dtpost_'.$part->id.'"'.$class.'>'.turnitintool_dateselect('dtpost',$date).'</div>';
            $cells[3].='<span id="dtposttext_'.$part->id.'"></span>';
            
            $cells[4]='<div id="maxmarks_'.$part->id.'"'.$class.'><input name="maxmarks" type="text" value="'.$part->maxmarks.'" /></div>';
            $cells[4].='<span id="maxmarkstext_'.$part->id.'"></span>';
            
            $cells[5]='<div id="tick_'.$part->id.'"><input type="image" src="images/tickicon.gif" /></div>';
            $cells[5].='<span id="ticktext_'.$part->id.'"></span>
            ';
            
            $cells[5].='<script language="javascript" type="text/javascript">
                values=new Array();
                values["partnametext"]="'.$part->partname.'";
                values["dtstarttext"]="'.userdate($part->dtstart,get_string('strftimedatetimeshort','langconfig')).'";
                values["dtduetext"]="'.userdate($part->dtdue,get_string('strftimedatetimeshort','langconfig')).'";
                values["dtposttext"]="'.userdate($part->dtpost,get_string('strftimedatetimeshort','langconfig')).'";
                values["maxmarkstext"]="'.$part->maxmarks.'";
                values["ticktext"]="<a href=\"javascript:;\" onclick=\"turnitintool_editpart(parts,'.$part->id.')\" title=\"'.get_string('edit','turnitintool').'\"><img src=\"images/editicon.gif\" class=\"tiiicons\" alt=\"'.get_string('edit','turnitintool').'\" /></a>";
                turnitintool_addvalues('.$part->id.',values);
            </script></form>';
            ...
            $cells[6]='<a href="'.$CFG->wwwroot.'/mod/turnitintool/view.php'.
                        '?id='.$cm->id.'&do=intro&delpart='.$part->id.'" title="'.get_string('delete','turnitintool').
                        '"'.$warning.'><img src="images/trashicon.gif" class="tiiicons" alt="'.get_string('delete','turnitintool').'" /></a>';
                        
        } else { // DO STUDENT VIEW
            $cells[0]=$part->partname;
            $cells[1]=userdate($part->dtstart,get_string('strftimedatetimeshort','langconfig'));
            $cells[2]=userdate($part->dtdue,get_string('strftimedatetimeshort','langconfig'));
            $cells[3]=userdate($part->dtpost,get_string('strftimedatetimeshort','langconfig'));
            $cells[4]=$part->maxmarks;
            $cells[5]='&nbsp;';
            $cells[6]='&nbsp;';
        }
        $table->data[]=$cells;
...
/**
 * Prints the select drop down and form to allow the tutor to change the Turnitin owner of a class 
 *
 * @param object $cm The moodle course module object for this instance
 * @param object $turnitintool The turnitintool object is for this activity
 * @return string Returns the output to print to screen
 */
function turnitintool_ownerselect($cm,$turnitintool) {
    global $CFG;
    // Get list of Tutors teaching on this course
    $context = get_context_instance(CONTEXT_MODULE, $cm->id);
    $allusers=get_users_by_capability($context, 'mod/turnitintool:grade', "u.id AS id,concat(concat(u.firstname,' '),u.lastname) AS name", '', '', '', '', '', false);
    $output='';
    if (count($allusers)>1) {
        $output.='<span style="display: none;" id="editOwner"><a href="javascript:toggleElements();" title="'.
                    get_string('edit','turnitintool').'"><img src="images/editicon.gif" class="tiiicons" /></a></span>
        <form action="'.$CFG->wwwroot.'/mod/turnitintool/view.php" method="get" id="ownerForm">';
        $output.='<input name="id" type="hidden" value="'.$cm->id.'" />';
        $output.='<select name="owner" onchange="turnitintool_jumptopage(\''.$CFG->wwwroot.'/mod/turnitintool/view.php?id='.$cm->id.'&do=changeowner&owner=\'+this.value)">';
        $output.='    <option label="'.get_string('changeowner','turnitintool').'" value="NULL" selected="selected">'.get_string('changeowner','turnitintool').'</option>';
        $output.='    <option label="------------------------------" value="NULL">-----------------------------</option>';
        foreach ($allusers as $thisuser) {
            $output.='    <option label="'.$thisuser->name.'" value="'.$thisuser->id.'">'.$thisuser->name.'</option>';
        }
        $output.='</select>';
        $output.='<input value=">" type="submit" id="submitButton" />';
        $output.='</form>';
        $output.='<script language="javascript">';
        $output.='toggleElements();';
        $output.='function toggleElements() {
...
function turnitintool_ownerprocess($cm,$turnitintool,$newid) {
    ...
    if ($newid!='NULL') {
        ...
        $context = get_context_instance(CONTEXT_MODULE, $cm->id);
        $allusers=get_users_by_capability($context, 'mod/turnitintool:grade', 'u.id AS id,concat(u.firstname,\' \',u.lastname) as name', '', '', '', 0, '', false);
        if (!isset($allusers[$newid])) {
            ...
            $post->new_teacher_email=$newowner->email;
            
            $tii->changeOwner($post,get_string('changingowner','turnitintool'));
            
            if ($tii->getRerror()) {
                $reason=($tii->getAPIunavailable()) ? get_string('apiunavailable','turnitintool') : $tii->getRmessage();
                turnitintool_print_error('<strong>'.get_string('turnitintoolupdateerror','turnitintool').'</strong><br />'.$reason,NULL,__FILE__,__LINE__);
                exit();
            } else {
...
function turnitintool_dateselect($fieldname,$selectedarray=NULL) {
    ...
    if (substr_count($format,"d")==0 OR substr_count($format,"m")==0 OR substr_count($format,"Y")==0) {
        $format="d/m/Y";
    }
    ...
    $i=0;
    $dateselect='';
    foreach ($formatsplit as $datekey) {
        if ($i!=0) {
            $dateselect.='/';
        }
    ...
    if (empty($submission->submission_objectid)) {
        ...
        if (!is_null($objectid) AND (has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id)) OR $turnitintool->studentreports)) {
            ...
            if ((!is_null($result) AND !empty($result)) OR $result=="0") {
                ...
                $style2="";
                $result.='%';
                $reportlink=$CFG->wwwroot.'/mod/turnitintool/view.php?id='.$cm->id.'&jumppage=report&userid='.$thisuser->id.'&post='.base64_encode(serialize($post));
...
function turnitintool_get_filelink($cm,$turnitintool,$submission,$download=false) {
    ...
    if (empty($submission->submission_objectid)) {
        ...
        $post->paperid=$submission->submission_objectid;
        if (!$download) {
            $filelink=$CFG->wwwroot.'/mod/turnitintool/view.php?id='.$cm->id.'&jumppage=submission&userid='.$thisuser->id.'&post='.base64_encode(serialize($post));
        } else {
            $filelink=$CFG->wwwroot.'/mod/turnitintool/view.php?id='.$cm->id.'&jumppage=download&userid='.$thisuser->id.'&post='.base64_encode(serialize($post));
        }
    }
    ...
    <table class="toplinkTabs">
        <tr>
            <td class="tabLinks"><ul class="tabrow0 tabRowModified">
                <li><a href="'.$CFG->wwwroot.'/mod/turnitintool/view.php'.'?id='.$cm->id.'&do='.$param_do.
                '&update=1"><span><img src="'.$CFG->wwwroot.'/mod/turnitintool/images/refresh.gif" alt="'.
            ...
            $table->cellpadding='4px';
            $table->cellspacing='0px';
            $table->head=array( get_string('submission','turnitintool'),
                                get_string('posted','turnitintool'),
            ...
            foreach ($submissions as $submission) {
                ...
                if (!is_null($submission->submission_objectid)) {
                    $download='<a href="'.turnitintool_get_filelink($cm,$turnitintool,$submission,true).'" title="'.
                                get_string('downloadsubmission','turnitintool').'" target="_blank"><img src="images/downloadicon.gif" alt="'.
                ...
                if (empty($submission->submission_objectid)
                    ...
                    OR ($turnitintool->reportgenspeed==1 AND ($turnitintool->allowlate OR $part->dtdue>time()))) {
                    $delete='<a href="'.$CFG->wwwroot.'/mod/turnitintool/view.php'.'?id='.$cm->id.'&delete='.$submission->id.
                            ...
                            '"><img src="images/trashicon.gif" alt="'.get_string('deletesubmission','turnitintool').'" class="tiiicons" /></a>';
                } else {
                    $delete='-';
                }
...
function turnitintool_overallgrade($turnitintool,$usersubmissions,$userid) {
    ...
    foreach ($usersubmissions[$userid] as $usersubmission) {
        if (!empty($usersubmission->submission_grade) AND !is_null($usersubmission->submission_grade) AND $weightarray[$usersubmission->submission_part]!=0) {
    ...
    if (!is_null($overallgrade)) {
        return number_format($overallgrade,1);
    } else {
        return '-';
    }
}
            ...
            foreach ($comments as $comment) {
                ...
                if (isset($post['action']) AND ($post['action']=='view' OR $post['action']=='edit') AND $post['comment']!=$comment->id) {
                    ...
                    $output.='<div class="commentBlock"><div class="commentLeft"><b>'.get_string('commentby','turnitintool').
                                ':</b> '.$commentuser->firstname.' '.$commentuser->lastname.'</div>';
                    $output.='<div class="commentRight"><b>'.get_string('posted','turnitintool').':</b> '.userdate($comment->date,get_string('strftimerecentfull','langconfig')).'</div>';
                    $output.='<hr class="commentRule" />';
                    $output.='<div class="commentComments">'.nl2br($comment->comment).'</div>';
                    $output.='<br />
                    <div class="clearBlock">&nbsp;</div>';
                    if ($comment->userid==$USER->id AND ($comment->date>=time()-$turnitintool->commentedittime
                    OR
                    has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id)))) {
                        ...
                        if (!has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id))) {
                            $output.='<span class="editNotice">'.get_string('edituntil','turnitintool').' '.userdate($comment->date+$turnitintool->commentedittime,get_string('strftimerecentfull','langconfig')).'</span>';
                        }
                        $output.='</div>
                        <div class="clearBlock">&nbsp;</div>';
                    }
...
function turnitintool_addedit_notes($cm,$turnitintool,$view,$post,$notice) {
    ...
    if ($submission->userid==$USER->id OR has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id))) {
        ...
        $cell[]=get_string('comment','turnitintool').' (<span id="charsBlock">'.strlen($comments).'/'.$turnitintool->commentmaxsize.'</span> chars)';
        $cell[]='<textarea class="submissionText" name="comments" onkeyup="turnitintool_countchars(this,\'charsBlock\','.$turnitintool->commentmaxsize.',\''.
        ...
        $cell[]='&nbsp;';
        $cell[]=$action.'<a href="'.$CFG->wwwroot.'/mod/turnitintool/view.php'.'?id='.$cm->id.'&do=notes&s='.$submission->id.'"><input name="cancel" value="Cancel" type="button" /></a>
...
function turnitintool_process_notes($cm,$turnitintool,$view,$post) {
    ...
    if ($submission->userid==$USER->id OR has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id))) { // Revalidate user
        ...
        if (isset($post["action"]) AND $post["action"]!="view") {
            ...
            if ($post["action"]=="delete" AND (($comment->userid==$USER->id AND $comment->date>=time()-$turnitintool->commentedittime) OR $isgrader)) {
                ...
                exit();
            } else if (($post["action"]=="edit" AND (($comment->userid==$USER->id AND $comment->date>=time()-$turnitintool->commentedittime) OR $isgrader)) OR $post["action"]=="add") { 
                if (empty($post["comments"])) {
                    ...
                    $insert->comment=strip_tags($post["comments"]);
                    $insert->date=time();
                    $insert->deleted=0;
                    turnitintool_insert_record('turnitintool_comments',$insert,false);
                    turnitintool_redirect($CFG->wwwroot.'/mod/turnitintool/view.php?id='.$cm->id.'&do=notes&s='.$submission->id);    
                    exit();
                }
            } else {
...
function turnitintool_get_moodleuser($userid,$arg1=NULL,$arg2=NULL,$arg3=NULL) {
    if (!$user=turnitintool_get_record('user','id',$userid)) {
        turnitintool_print_error('usergeterror','turnitintool','',$arg1,$arg2,$arg3);
        exit();
    }
    if (empty($user->email)) {
...
function turnitintool_view_options($cm,$turnitintool) {
    ...
    unset($cell);
    $cell[0]=get_string('autosubmit','turnitintool').turnitintool_help_icon('autosubmit',get_string('autosubmit','turnitintool'),'turnitintool',true,false,'',true);
    $cell[1]='<select name="autosubmission" class="formwide">';
    
    $cell[1].='<option label="'.get_string('autosubmiton','turnitintool').'" value="1"'.$selected['1'].'>'.get_string('autosubmiton','turnitintool').'</option>';
    
    $cell[1].='<option label="'.get_string('autosubmitoff','turnitintool').'" value="0"'.$selected['0'].'>'.get_string('autosubmitoff','turnitintool').'</option>';
    $cell[1].='</select>';
    $table->data[]=$cell;
    ...
    if ($CFG->turnitin_usegrademark) {
        ...
        unset($cell);
        $cell[0]=get_string('turnitinusegrademark','turnitintool').turnitintool_help_icon(  'turnitinusegrademark',
        ...
        $cell[1]='<select name="usegrademark" class="formwide">';
        $cell[1].='<option label="'.get_string('yesgrademark','turnitintool').'" value="1"'.$selected['1'].'>'.get_string('yesgrademark','turnitintool').'</option>';
        $cell[1].='<option label="'.get_string('nogrademark','turnitintool').'" value="0"'.$selected['0'].'>'.get_string('nogrademark','turnitintool').'</option>';
        $cell[1].='</select>';
        $table->data[]=$cell;
    ...
    unset($cell);
    $cell[0]=get_string('displaygradesas','turnitintool').turnitintool_help_icon('displaygradesas',get_string('displaygradesas','turnitintool'),'turnitintool',true,false,'',true);
    $cell[1]='<select name="gradedisplay" class="formwide">';
    $cell[1].='<option label="'.get_string('displaygradesaspercent','turnitintool').'" value="1"'.$selected['1'].'>'.get_string('displaygradesaspercent','turnitintool').'</option>';
    $cell[1].='<option label="'.get_string('displaygradesasfraction','turnitintool').'" value="2"'.$selected['2'].'>'.get_string('displaygradesasfraction','turnitintool').'</option>';
    $cell[1].='</select>';
    $table->data[]=$cell;
    ...
    unset($cell);
    $cell[0]=get_string('autorefreshgrades','turnitintool').turnitintool_help_icon('autorefreshgrades',get_string('autorefreshgrades','turnitintool'),'turnitintool',true,false,'',true);
    $cell[1]='<select name="autoupdates" class="formwide">';
    $cell[1].='<option label="'.get_string('yesgrades','turnitintool').'" value="1"'.$selected['1'].'>'.get_string('yesgrades','turnitintool').'</option>';
    $cell[1].='<option label="'.get_string('nogrades','turnitintool').'" value="0"'.$selected['0'].'>'.get_string('nogrades','turnitintool').'</option>';
    $cell[1].='</select>';
    $table->data[]=$cell;
    ...
    unset($cell);
    $cell[0]=get_string('submissionlist','turnitintool').turnitintool_help_icon('submissionlist',get_string('submissionlist','turnitintool'),'turnitintool',true,false,'',true);
    $cell[1]='<select name="shownonsubmission" class="formwide">';
    
    $cell[1].='<option label="'.get_string('shownonsubmissions','turnitintool').'" value="1"'.$selected['1'].'>'.get_string('shownonsubmissions','turnitintool').'</option>';
    
    $cell[1].='<option label="'.get_string('showonlysubmissions','turnitintool').'" value="0"'.$selected['0'].'>'.get_string('showonlysubmissions','turnitintool').'</option>';
    $cell[1].='</select>';
    $table->data[]=$cell;
    ...
    $cell[0]=get_string('commenteditwindow','turnitintool').turnitintool_help_icon('commenteditwindow',get_string('commenteditwindow','turnitintool'),'turnitintool',true,false,'',true);
    $cell[1]='<select name="commentedittime" class="formwide">';
    
    $cell[1].='<option label="'.get_string('nolimit','turnitintool').'" value="0"'.$selected['0'].'>'.get_string('nolimit','turnitintool').'</option>';
    $cell[1].='<option label="5 '.get_string('minutes','turnitintool').'" value="300"'.$selected['300'].'>5 '.get_string('minutes','turnitintool').'</option>';
    $cell[1].='<option label="10 '.get_string('minutes','turnitintool').'" value="600"'.$selected['600'].'>10 '.get_string('minutes','turnitintool').'</option>';
    $cell[1].='<option label="15 '.get_string('minutes','turnitintool').'" value="900"'.$selected['900'].'>15 '.get_string('minutes','turnitintool').'</option>';
    $cell[1].='<option label="20 '.get_string('minutes','turnitintool').'" value="1200"'.$selected['1200'].'>20 '.get_string('minutes','turnitintool').'</option>';
    $cell[1].='<option label="25 '.get_string('minutes','turnitintool').'" value="1500"'.$selected['1500'].'>25 '.get_string('minutes','turnitintool').'</option>';
    $cell[1].='<option label="30 '.get_string('minutes','turnitintool').'" value="1800"'.$selected['1800'].'>30 '.get_string('minutes','turnitintool').'</option>';
    $cell[1].='<option label="1 '.get_string('hours','turnitintool').'" value="3600"'.$selected['3600'].'>1 '.get_string('hours','turnitintool').'</option>';
    $cell[1].='<option label="2 '.get_string('hours','turnitintool').'" value="7200"'.$selected['7200'].'>2 '.get_string('hours','turnitintool').'</option>';
    $cell[1].='<option label="3 '.get_string('hours','turnitintool').'" value="10800"'.$selected['10800'].'>3 '.get_string('hours','turnitintool').'</option>';
    $cell[1].='<option label="4 '.get_string('hours','turnitintool').'" value="14400"'.$selected['14400'].'>4 '.get_string('hours','turnitintool').'</option>';
    $cell[1].='<option label="5 '.get_string('hours','turnitintool').'" value="18000"'.$selected['18000'].'>5 '.get_string('hours','turnitintool').'</option>';
    $cell[1].='<option label="6 '.get_string('hours','turnitintool').'" value="21600"'.$selected['21600'].'>6 '.get_string('hours','turnitintool').'</option>';
    $cell[1].='<option label="12 '.get_string('hours','turnitintool').'" value="43200"'.$selected['43200'].'>12 '.get_string('hours','turnitintool').'</option>';
    $cell[1].='<option label="24 '.get_string('hours','turnitintool').'" value="86400"'.$selected['86400'].'>24 '.get_string('hours','turnitintool').'</option>';
    ...
    $cell[0]=get_string('maxcommentsize','turnitintool').turnitintool_help_icon('maxcommentsize',get_string('maxcommentsize','turnitintool'),'turnitintool',true,false,'',true);
    $cell[1]='<select name="commentmaxsize" class="formwide">';
    
    for ($i=1;$i<=15;$i++) {
        $n=$i*100;
        $cell[1].='<option label="'.$n.' '.get_string('characters','turnitintool').'" value="'.$n.'"'.$selected[$n].'>'.$n.' '.get_string('characters','turnitintool').'</option>';
    }
    ...
    unset($cell);
    $cell[0]='';
    $cell[1]='<input type="submit" value="'.get_string('savechanges','turnitintool').'" />';
    $table->data[]=$cell;
...
function turnitintool_doheaderlinks($cm,$turnitintool,$title,$hilo,$lohi) {
    ...
    $headerlink='<a href="'.$CFG->wwwroot.'/mod/turnitintool/view.php?id='.$cm->id.'&do='.$param_do.'&ob=%%OB%%">'.$title.'</a>';
    
    if (isset($param_ob) AND $param_ob==$hilo) {
        $headerlink.=' <img src="images/order_down.gif" class="ordericons" />';
        $headerlink=str_replace('%%OB%%',$lohi,$headerlink);
    } else if (isset($param_ob) AND $param_ob==$lohi) {
        $headerlink.=' <img src="images/order_up.gif" class="ordericons" />';
 ...
 */
function turnitintool_revealuser($cm,$turnitintool,$post) {
    global $CFG;
    $anonid=$post["anonid"];
    ...
    if ($reason==get_string('revealreason','turnitintool') OR empty($reason)) {
        ...
        $tiipost->anon_reason=$reason;
        
        $input->from=1;
        $input->to=1;
        $result=$tii->revealAnon($tiipost,get_string('updatestudent','turnitintool'));
        
        if ($tii->getRerror()) {
            $reason=($tii->getAPIunavailable()) ? get_string('apiunavailable','turnitintool') : $tii->getRmessage();
            turnitintool_print_error('<strong>'.get_string('submissionupdateerror','turnitintool').'</strong><br />'.$reason,NULL,'',NULL,__FILE__,__LINE__);
            exit();
        }
...
function turnitintool_view_all_submissions($cm,$turnitintool,$orderby='1') {
    ...
    $output='';
    $param_do=optional_param('do');
    $param_reason=optional_param('reason',array());
    
    // Define the tables I am using (Makes for easier switching when debugging problems)
    $table1=$CFG->prefix.'turnitintool_submissions';
    $table2=$CFG->prefix.'user';
    $table3=$CFG->prefix.'turnitintool';
    $table4=$CFG->prefix.'turnitintool_parts';
    ...
    foreach ($studentroles as $studentrole) {
        ...
        $roleid.="r.roleid=".$studentrole->id."";
    }
    // Get the course context object needed for the roles sql query part
    $context = get_context_instance(CONTEXT_COURSE, $turnitintool->course);
    $collation='';
    if ($CFG->dbfamily!='oracle') {
        // Determine the collation of the moodle database (necessary for union of slightly different text encoding, query ID for first field 'cause Moodle says so')
        $collation_query=turnitintool_get_records_sql("SELECT id,collation(firstname) collation FROM ".$CFG->prefix."user");
        $collation_obj=current($collation_query);
        $collation=' COLLATE '.$collation_obj->collation;
    }
    ...
    if ($turnitintool->numparts!=1 AND !$turnitintool->anon) { // If Multi Part and NOT Anonymous
        if ($turnitintool->shownonsubmission) {
            // ## NOTICE: The UNIONs in the inbox database calls are pretty large.
            // ##         At some point these should be optimised to make them less expensive.
            
            // Get all students submitters and non submitters
            $sql_query.="(SELECT u.id AS userid,
                                 u.firstname".$collation." AS firstname,
                                 u.lastname".$collation." AS lastname,
                                 AVG(s.submission_score) avscore,
                                 AVG(s.submission_grade) avgrade,
                                 0 nonmoodle,
                                 MAX(s.submission_unanon) unanon,
                                 COUNT(s.submission_modified) count,
                                 MAX(s.submission_score) highscore,
                                 MIN(s.submission_score) lowscore,
                                 MIN(s.submission_modified) mindate,
                                 MAX(s.submission_modified) maxdate
                            FROM ".$table5." r JOIN ".$table2." u
                                ON u.id=r.userid
                                LEFT OUTER JOIN ".$table1." s
                                ON s.userid=u.id AND s.turnitintoolid=".$turnitintool->id."
                            WHERE r.contextid=".$context->id."
                            AND (".$roleid.") GROUP BY 1)
                        UNION ";
        } else {
            // Get just students that have made submissions
            $sql_query.="(SELECT s.userid AS userid,
                                 u.firstname".$collation." AS firstname,
                                 u.lastname".$collation." AS lastname,
                                 AVG(s.submission_score) avscore,
                                 AVG(s.submission_grade) avgrade,
                                 ...
                                 MIN(s.submission_modified) mindate,
                                 MAX(s.submission_modified) maxdate
                            FROM ".$table1." s JOIN ".$table2." u
                                ON u.id=s.userid
                            WHERE s.userid!=0 AND s.turnitintoolid=".$turnitintool->id."
                            GROUP BY 1)
                        UNION ";
        }
        
        // Now get submissions that were not made by a non moodle student
        $sql_query.="(SELECT concat('nm-',s.submission_nmuserid)".$collation." AS userid,
                             s.submission_nmfirstname".$collation." AS firstname,
                             s.submission_nmlastname".$collation." AS lastname,
                             AVG(s.submission_score) avscore,
                             AVG(s.submission_grade) avgrade,
                             1 nonmoodle,
                             MAX(s.submission_unanon) unanon,
        ...
        // ORDER BY: 1: Av. Originality Score - Max-Max,
        //           2: Av. Originality Score - Min-Max,
        //           3: Av. Grade - Max-Min,
        //           4: Av. Grade - Min-Max,
        ...
        if ($orderby=='1') {
            $order_by=' ORDER BY avscore DESC, lastname ASC';
            $sub_order='submission_score DESC';
        } else if ($orderby=='2') {
            $order_by=' ORDER BY avscore ASC, lastname ASC';
            $sub_order='submission_score ASC';
        } else if ($orderby=='3') {
            $order_by=' ORDER BY avgrade DESC, lastname ASC';
            $sub_order='submission_grade DESC';
        } else if ($orderby=='4') {
            $order_by=' ORDER BY avgrade ASC, lastname ASC';
            $sub_order='submission_grade ASC';
        } else if ($orderby=='5') {
            $order_by=' ORDER BY maxdate DESC, lastname ASC';
            $sub_order='submission_modified DESC';
        } else if ($orderby=='6') {
            $order_by=' ORDER BY mindate ASC, lastname ASC';
            $sub_order='submission_modified ASC';
        } else if ($orderby=='7') {
            $order_by=' ORDER BY lastname DESC';
            $sub_order='submission_title DESC';
        } else if ($orderby=='8') {
            $order_by=' ORDER BY lastname ASC';
            $sub_order='submission_title ASC';
        }
        
        if (!$users=turnitintool_get_records_sql($sql_query.$order_by)) {
            $users=array();
        } else {
            $noresults=false;
            // Pass some user/submission array values from PHP to javascript [[[[
            $javascriptUser='';
            $javascriptCount='';
            $i=1;
            foreach ($users as $key=>$values) {
                ...
                $javascriptUser.='"'.$values->userid.'"'.$addcomma;
                $javascriptCount.='"'.$values->count.'"'.$addcomma;
                $i++;
            }
            $output.= $styles.'
        <script language="javascript" type="text/javascript">
            var users= new Array('.$javascriptUser.');
        ...
        foreach ($users as $key=>$values) {
            ...
            $sub_query='SELECT * FROM '.$table1.' WHERE '.$where.' ORDER BY '.$sub_order;
            if ($submissions=turnitintool_get_records_select('turnitintool_submissions',$where,$sub_order)) {
                if ($values->count>0) { // Draw Plus Link if there is more than one submission
                    ...
                    <img src="images/clearpixel.gif" class="plusminus" id="userblock_'.$values->userid.'" />
                    <script language="javascript" type="text/javascript">
                    document.write(\'</a>\');
                    document.getElementById(\'userblock_'.$values->userid.'\').src="images/plus.gif";
                    </script>';
                ...
                if ($values->nonmoodle) {
                    $cells['student']='<div class="student"><i>'.$pluslink.$values->lastname.', '.$values->firstname.
                                        ' ('.get_string('nonmoodleuser','turnitintool').')</i> - ('.$values->count.
                                        ' '.$submissionstring.')</div>';
                } else {
                    $cells['student']='<div class="student">'.$pluslink.'<a href="'.$CFG->wwwroot.'/user/view.php?id='.
                                        $values->userid.'&course='.$turnitintool->course.'">'.$values->lastname.', '.$values->firstname.
                                        '</a> - ('.$values->count.' '.$submissionstring.')</div>';
                }
                $cells['modified']='<div class="student">&nbsp;</div>';
                
                $score=($values->lowscore==$values->highscore OR empty($values->lowscore)) ? $values->highscore.'%' : $values->lowscore.'%>'.$values->highscore.'%';
                if ($score=='%') {
                    $score='-';
                }
                $cells['score']='<div class="student">'.$score.'</div>';
                ...
                if ($overallgrade!='-' AND $turnitintool->gradedisplay==2) { // 2 is fraction
                    $overallgrade.='/'.$turnitintool->grade;
                } else if ($overallgrade!='-' AND $turnitintool->gradedisplay==1) { // 1 is percentage
                    $overallgrade=round($overallgrade/$turnitintool->grade*100,1).'%';
                }
                $cells['grade']='<div class="student">'.$overallgrade.'</div>';
                ...
                foreach ($submissions as $submission) {
                    ...
                    $cells['student']='<div id="toggle_'.$values->userid.'_'.$i.'_1" class="submission'.$lastclass.'"><a href="'.$filelink.
                                        '" target="_blank" class="fileicon"'.$doscript.'>'.$part->partname.' - '.$submission->submission_title.'</a></div>';
                    ...
                    $cells['modified']='<div id="toggle_'.$values->userid.'_'.$i.'_2" class="submission">'.$modified.'</div>';
                    
                    // ###################################
                    // Get originality score if available
                    // ###################################
                    $score = turnitintool_draw_similarityscore($cm,$turnitintool,$submission);
                    $cells['score']='<div id="toggle_'.$values->userid.'_'.$i.'_3" class="submission">'.$score.'</div>';

                    // ###################################
                    // Get grade if available
                    // ###################################
                    $grade=turnitintool_dogradeoutput($cm,$turnitintool,$submission);
                    $cells['grade']='<div id="toggle_'.$values->userid.'_'.$i.'_4" class="submission"><form action="'.$CFG->wwwroot.
                                    '/mod/turnitintool/view.php'.'?id='.$cm->id.'&do=allsubmissions" method="POST">'.$grade.'</form></div>';
                    ...
                    $cells['feedback']='<div id="toggle_'.$values->userid.'_'.$i.'_5" class="submission">'.$notes.'</div>';
                    ...
                    $cells['download']='<div id="toggle_'.$values->userid.'_'.$i.'_6" class="submission" target="_blank">'.$download.'</div>';
                    ...
                    if (empty($submission->submission_objectid) OR has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id))) {
                        $delete='<a href="'.$CFG->wwwroot.'/mod/turnitintool/view.php'.'?id='.$cm->id.'&delete='.$submission->id.'&do='.$param_do.
                                '"'.$confirm.' title="'.get_string('deletesubmission','turnitintool').'"><img src="images/trashicon.gif" alt="'.
                                get_string('deletesubmission','turnitintool').'" class="tiiicons" /></a>';
                    } else {
                        $delete='-';
                    }
                    $cells['delete']='<div id="toggle_'.$values->userid.'_'.$i.'_7" class="submission">'.$delete.'</div>';
                    $output_array[]=$cells;
                }
            } else {
                $pluslink='
                ...
                $cells['student']='<div class="student">'.$pluslink.'<a href="'.$CFG->wwwroot.'/user/view.php?id='.$values->userid.'&course='.
                                    $turnitintool->course.'">'.$values->lastname.', '.$values->firstname.'</a> - ('.$values->count.' '.
    ...
    } else if (!$turnitintool->anon) { // If Single Part and NOT Anonymous
        if ($turnitintool->shownonsubmission) {
            // Get all students submitters and non submitters
            $sql_query.="(SELECT COALESCE(s.id,u.username) AS uindex,
                                 u.id AS userid,
                                 u.firstname".$collation." AS firstname,
                                 u.lastname".$collation." AS lastname,
                                 s.submission_score submission_score,
                                 s.submission_grade submission_grade,
                                 s.submission_objectid submission_objectid,
                                 0 nonmoodle,
                                 s.id id,
                                 s.submission_part submission_part,
                                 s.submission_filename submission_filename,
                                 s.submission_title submission_title,
                                 s.submission_gmimaged submission_gmimaged,
                                 s.submission_modified submission_modified
                            FROM ".$table5." r JOIN ".$table2." u
                               ON u.id=r.userid LEFT OUTER JOIN ".$table1." s
                               ON s.userid=u.id AND s.turnitintoolid=".$turnitintool->id."
                            WHERE r.contextid=".$context->id." AND (".$roleid.")
                            GROUP BY 1)
                        UNION ";
        } else {
            // Get just students that have made submissions
            $sql_query.="(SELECT s.id AS uindex,
                                 s.userid AS userid,
                                 u.firstname".$collation." AS firstname,
                                 u.lastname".$collation." AS lastname,
                                 s.submission_score submission_score,
        ...
        // Now get submissions that were not made by a non moodle student
        $sql_query.="(SELECT s.id AS uindex,
                             concat('nm-',s.submission_nmuserid)".$collation." AS userid,
                             s.submission_nmfirstname".$collation." AS firstname,
                             s.submission_nmlastname".$collation." AS lastname,
                             s.submission_score submission_score,
                             s.submission_grade submission_grade,
        ...
        if ($orderby=='1') {
            $order_by=' ORDER BY submission_score DESC,submission_modified DESC,lastname ASC';
        } else if ($orderby=='2') {
            $order_by=' ORDER BY submission_score ASC,submission_modified DESC,lastname ASC';
        } else if ($orderby=='3') {
            $order_by=' ORDER BY submission_grade DESC,submission_score DESC,lastname ASC';
        } else if ($orderby=='4') {
            $order_by=' ORDER BY submission_grade ASC,submission_score DESC,lastname ASC';
        } else if ($orderby=='5') {
            $order_by=' ORDER BY submission_score DESC,submission_modified DESC,lastname ASC';
        } else if ($orderby=='6') {
            $order_by=' ORDER BY submission_score DESC,submission_modified ASC,lastname ASC';
        } else if ($orderby=='7') {
            $order_by=' ORDER BY lastname DESC,submission_score DESC';
        } else if ($orderby=='8') {
            $order_by=' ORDER BY lastname ASC,submission_score DESC';
        }
        
        if (!$submissions=turnitintool_get_records_sql($sql_query.$order_by)) {
            $submissions=array();
        } else {
            $noresults=false;
        }
        ...
        foreach ($submissions as $submission) {
            if (is_null($submission->id)) {
                $cells['student']='<div class="student"><b>'.get_string('file').':</b> '.get_string('nosubmission','turnitintool').
                                    '<br /><b>'.get_string('student','turnitintool').':</b> <a href="'.$CFG->wwwroot.'/user/view.php?id='.$submission->userid.
                                    '&course='.$turnitintool->course.'">'.$submission->lastname.', '.$submission->firstname.'</a></div>';
                ...
                // Output the student / Submission Cell contents after determining if it is a moodle user submission or turnitin only submission
                if (!$submission->nonmoodle) {
                    $namedisplay='<a href="'.$CFG->wwwroot.'/user/view.php?id='.$submission->userid.'&course='.$turnitintool->course.'">'.
                                    $submission->lastname.', '.$submission->firstname.'</a>';
                } else {
                    $namedisplay='<i>'.$submission->lastname.', '.$submission->firstname.' ('.get_string('nonmoodleuser','turnitintool').')</i>';
                }
                $cells['student']='<div class="student"><b>'.get_string('file').':</b> <a href="'.$filelink.'" target="_blank" class="tiiicons"'.
                                    $doscript.'>'.$submission->submission_title.'</a><br /><b>'.get_string('student','turnitintool').'</b>: '.$namedisplay.'</div>';
                // ###########################################
                ...
                $cells['modified']='<div class="student">'.$modified.'</div>';
                ...
                // ###################################
                $grade=turnitintool_dogradeoutput($cm,$turnitintool,$submission);
                $cells['grade']='<div class="student"><form action="'.$CFG->wwwroot.'/mod/turnitintool/view.php'.'?id='.$cm->id.
                                    '&do=allsubmissions" method="POST">'.$grade.'</form></div>';
                ...
                $cells['feedback']='<div class="student">'.$notes.'</div>';
                ...
                $cells['download']='<div class="student">'.$download.'</div>';
                
                if (empty($submission->submission_objectid) OR has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id))) {
                    $delete='<a href="'.$CFG->wwwroot.'/mod/turnitintool/view.php'.'?id='.$cm->id.'&delete='.$submission->id.
                            '&do='.$param_do.'"'.$confirm.' title="'.get_string('deletesubmission','turnitintool').'"><img src="images/trashicon.gif" alt="'.
                            get_string('deletesubmission','turnitintool').'" class="tiiicons" /></a>';
                } else {
                    $delete='-';
                }
                $cells['delete']='<div class="student">'.$delete.'</div>';
                $output_array[]=$cells;
            }
        }
    } else { // If Anonymous
        $javascriptUser=''; // Initialise the Choice Array for User (Part)
        $javascriptCount=''; // Initialise the Choice Array for Count
        $n=0;

        $sql_query='';
        if ($turnitintool->shownonsubmission) { // If we are displaying students that have not made submissions
            
        }

        $sql_query.="(SELECT s.id id,
                            s.userid AS userid,
                             u.firstname".$collation." AS firstname,
                             u.lastname".$collation." AS lastname,
                             s.submission_score submission_score,
                             ...
                             s.submission_title submission_title,
                             s.submission_gmimaged submission_gmimaged,
                             s.submission_unanon unanon,
                             s.submission_modified submission_modified
                         FROM ".$table1." s 
                            JOIN ".$table2." u
                            ON u.id=s.userid
                         WHERE s.userid!=0 
                            AND s.turnitintoolid=".$turnitintool->id.")
                    UNION ";
        ...
        if ($orderby=='1') {
            ...
            $order_by=' ORDER BY submission_part ASC,submission_score DESC,submission_modified ASC,lastname ASC';
        } else if ($orderby=='7') {
        ...
        if (!$parts=turnitintool_get_records_select('turnitintool_parts','turnitintoolid='.$turnitintool->id.' AND deleted=0','dtstart,dtdue,dtpost,id')) {
            ...
            $noresults=false; // Do not show the no results table row as we have results
        }
        ...
        foreach ($parts as $part) {
            ...
            if ($numsubs>0) { // Draw Plus Link if there is more than one submission
                $pluslink='
                <script language="javascript" type="text/javascript">
                document.write("<a href=\"javascript:;\" onclick=\"toggleview(\''.$part->id.'\',\''.count($submissions).'\');\">");
                </script>
                <img src="images/clearpixel.gif" class="plusminus" id="userblock_'.$part->id.'" />
                <script language="javascript" type="text/javascript">
                document.write(\'</a>\');
                document.getElementById(\'userblock_'.$part->id.'\').src="images/plus.gif";
                </script>';
            } else { // Draw a place holder spacer if there are less than one submissions
                $pluslink='
                <img src="images/clearpixel.gif" class="plusminus" id="userblock_'.$part->id.'" />
            ...
            foreach ($submissions as $submission) {
                ...
                if ($part->id==$submission->submission_part) {
                    ...
                    $i++;
                    $lastclass=($i==count($submissions)) ? ' lastmark' : ' leftmark';
            ...
            toggleAnonView('.$submission->submission_objectid.',false);
                    ...
                    if (!empty($submission->submission_title)) {
                        ...
                        $title='<b>'.get_string('file','turnitintool').':</b> <a href="'.$filelink.'" target="_blank" class="tiiicons"'.$doscript.
                                '>'.$submission->submission_title.'</a><br />';
                    } else {
                        $title='<b>'.get_string('file','turnitintool').':</b> '.get_string('nosubmission','turnitintool').'<br />';
                    }
                    $cells['student']='<div id="toggle_'.$part->id.'_'.$i.'_1" class="submission'.$lastclass.'">'.$title.$namedisplay.'</div>';
                    ...
                    $cells['modified']='<div id="toggle_'.$part->id.'_'.$i.'_2" class="submission">'.$modified.'</div>';
                    
                    // ###################################
                    // Get originality score if available
                    // ###################################
                    $score = turnitintool_draw_similarityscore($cm,$turnitintool,$submission);
                    $cells['score']='<div id="toggle_'.$part->id.'_'.$i.'_3" class="submission">'.$score.'</div>';
        
                    // ###################################
                    // Get grade if available
                    // ###################################
                    $grade=turnitintool_dogradeoutput($cm,$turnitintool,$submission);
                    $cells['grade']='<div id="toggle_'.$part->id.'_'.$i.'_4" class="submission"><form action="'.$CFG->wwwroot.
                                    '/mod/turnitintool/view.php'.'?id='.$cm->id.'&do=allsubmissions" method="POST">'.$grade.'</form></div>';
                    ...
                    $cells['feedback']='<div id="toggle_'.$part->id.'_'.$i.'_5" class="submission">'.$notes.'</div>';
                    ...
                    $cells['download']='<div id="toggle_'.$part->id.'_'.$i.'_6" class="submission">'.$download.'</div>';
                    ...
                    if ((empty($submission->submission_objectid) OR has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id))) AND !is_null($submission->id)) {
                        ...
                        $delete='<a href="'.$CFG->wwwroot.'/mod/turnitintool/view.php'.'?id='.$cm->id.'&delete='.$submission->id.'&do='.
                                $param_do.'"'.$confirm.' title="'.get_string('deletesubmission','turnitintool').'"><img src="images/trashicon.gif" alt="'.
                                get_string('deletesubmission','turnitintool').'" class="tiiicons" /></a>';
                                
                    } else {
                        $delete='-';
                    }
                    $cells['delete']='<div id="toggle_'.$part->id.'_'.$i.'_7" class="submission">'.$delete.'</div>';
                    $output_array[]=$cells;
                }
            
            }
        
            // # Construct Javascript Choice Array START #
            $n++;
            if ($n==count($parts)) {
                $addcomma='';
            } else {
            ...
            $javascriptUser.='"'.$part->id.'"'.$addcomma;
            $javascriptCount.='"'.$numsubs.'"'.$addcomma;
            // # Construct Javascript Choice Array END #
...
function turnitintool_draw_submission_table($cm, $turnitintool, $input=array()) {
    ...
    $table->cellspacing='0px';
    $table->size=array('','12%','8%','10%','4%','4%','4%');
    $table->align=array('left','center','center','center','center','center','center');
    ...
    $table->head=array($student_header,
                            $modified_header,
                            $submissionorig_header,
                            $submissiongrade_header,
                            $feedback_header,
                            '',
                            ''
                            );
    foreach ($input as $row) {
        ...
        $cell6=$row['download'];
        $cell7=$row['delete'];
        $table->data[]=array($cell1,$cell2,$cell3,$cell4,$cell5,$cell6,$cell7);
    }
    
    // Draw Table Tabs and Toggle Links
    
    $output='<table class="toplinkTabs"><tr><td class="toggleLinks">';
    if ($turnitintool->numparts!=1 OR $turnitintool->anon) {
    ...
    $output.='</td><td class="tabLinks"><ul class="tabrow0 tabRowModified">
    <li><a href="'.$CFG->wwwroot.'/mod/turnitintool/view.php'.'?id='.$cm->id.'&do='.$param_do.'&update=1"><span><img src="'.
    $CFG->wwwroot.'/mod/turnitintool/images/refresh.gif" alt="'.get_string('turnitinrefreshsubmissions','turnitintool').
    '" class="tiiicons" />'.get_string('turnitinrefreshsubmissions','turnitintool').'</span></a></li>
    <li><a href="'.$CFG->wwwroot.'/mod/turnitintool/view.php'.'?id='.$cm->id.'&do='.$param_do.'&enroll=1"><span><img src="'.
    $CFG->wwwroot.'/mod/turnitintool/images/enrollicon.gif" alt="'.get_string('turnitinenrollstudents','turnitintool').'" class="tiiicons" />'.
    get_string('turnitinenrollstudents','turnitintool').'</span></a></li></ul></td></tr></table>';
...
function turnitintool_getnoteslink($cm,$turntintool,$submission) {
    global $CFG;
...
function turnitintool_enroll_all_students($cm,$turnitintool) {
    ...
    if (count($courseusers)>0) {
        ...
        foreach ($courseusers as $courseuser) {
            ...
            if ($tii->getRerror()) {
                $reason=($tii->getAPIunavailable()) ? get_string('apiunavailable','turnitintool') : $tii->getRmessage();
                turnitintool_print_error($reason,NULL,'',NULL,__FILE__,__LINE__);
                exit();
            }
            $tii->endSession();
...
function turnitintool_dogradeoutput($cm,$turnitintool,$submission,$textcolour='#666666',$background='transparent',$gradeable=true) {
    ...
    if ($CFG->turnitin_usegrademark AND $turnitintool->usegrademark AND ($post->utp==2 OR ($post->utp==1 AND $part->dtpost<=time()))) {
        ...
        $grademarkurl=$CFG->wwwroot.'/mod/turnitintool/view.php?id='.$cm->id.'&jumppage=grade&userid='.$thisuser->id.'&post='.base64_encode(serialize($post));    
        
        $doscript='';
        if (!empty($submission->submission_objectid)) {
			$warn=($turnitintool->reportgenspeed==1) ? $warn=',\''.get_string('resubmissiongradewarn','turnitintool').'\'' : '';
        ...
        if (!empty($submission->submission_objectid) AND ($post->utp==2 OR ($post->utp==1 AND $part->dtpost<=time()))) {
            if (!$submission->submission_gmimaged) {
                $output.='<img src="images/grademarkapple_grey.gif" class="tiiicons" />';
            } else {
                $output.='<a href="'.$grademarkurl.'"'.$doscript.'><img src="images/grademarkapple.gif" class="tiiicons" /></a>';
            }
        }
        ...
        if ($post->utp==2 AND $gradeable) {
            ...
            $output.='<input src="images/tickicon.gif" name="updategrade" value="updategrade" id="tick_'.$submission->id.'" class="tiiicons" type="image" />';
            $output.='<script language="javascript" type="text/javascript">
        ...
        if ($thisgrade!=$submission->submission_grade) {
            ...
            if ($tii->getRerror()) {
                if ($tii->getAPIunavailable()) {
                    ...
                    turnitintool_print_error($tii->getRmessage().' CODE: '.$tii->getRcode(),NULL,'',NULL,__FILE__,__LINE__);
                }
                exit();
            }
 ...
 * @param object $cm The moodle course module object for this instance
 * @param object $turnitintool The turnitintool object for this activity
 * @param boolean $forced If false then the Session Variable updatedscores is used to determine if the grades should update or true to force the update
 * @param object $loaderbar The loaderbar object passed by reference can be NULL if no loader bar is used
 */
function turnitintool_update_all_report_scores($cm,$turnitintool,$forced,&$loaderbar) {

    global $USER,$CFG,$notice;
    $param_type=optional_param('type');
    $param_do=optional_param('do');
    ...
    if (((isset($_SESSION['updatedscores'][$turnitintool->id]) AND $_SESSION['updatedscores'][$turnitintool->id]==1) OR isset($param_type)) AND !$forced) {
        return false;
    } else {
        ...
        if (has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id)) OR $turnitintool->studentreports OR $forced) {
            ...
            foreach ($parts as $part) {
                ...
                $post->assign=$turnitintool->name.' - '.turnitintool_partnamefromnum($part->id).' (Moodle '.$post->assignid.')';
                ...
                if ($tii->getRerror()) {
                    ...
                    if (!$ids=turnitintool_get_records_select('turnitintool_submissions','turnitintoolid='.$turnitintool->id.' AND submission_objectid IS NOT NULL AND submission_objectid!=\'\'','','submission_objectid,id,submission_grade,submission_status')) {
                        $ids=array();
                    }
                    
                    // Purge old submissions listings and insert the new ones
                    turnitintool_delete_records_select('turnitintool_submissions','submission_objectid IS NOT NULL AND submission_objectid!=\'\' AND submission_part='.$part->id);
                    // Now insert the submissions
                    ...
                    foreach ($resultArray as $key => $value) {
                            ...
                            if ($userdata=turnitintool_get_record('turnitintool_users','turnitin_uid',$value["userid"])) {
                                // If returned userid is already stored we can use real Moodle user to store against
                                ...
                                // If this user is a Moodle (TII linked) user and has submitted through TII to this class but is not enrolled on this class enroll them
                                if ($thisuser=turnitintool_get_moodleuser($userdata->userid,NULL,__FILE__,__LINE__) 
                                        AND count(turnitintool_search_users($turnitintool->course,'',$thisuser->email))<1) {
                                        
                                    enrol_into_course($course,$thisuser,'turnitintool');
                                }
                            ...
                            // Repatriate the feedback comments with the corresponding TII submission
                            if (isset($ids[$key])) {
                                if ($comments=turnitintool_get_records('turnitintool_comments','submissionid',$ids[$key]->id)) {
                                    foreach ($comments as $comment) {
                                        $update->id=$comment->id;
                                        $update->submissionid=$insertid;
                                        turnitintool_update_record('turnitintool_comments',$update);
                                    }
                                }
                            }
            ...
            if (isset($return)) {
                ...
                $_SESSION['updatedscores'][$turnitintool->id]='1';
                return true;
            }
        }
...
/**
 * Updates the LMS data with the Turnitin data and log the user into the originality report on Turnitin for this submission
 *
 * @param object $cm The moodle course module object for this instance
 * @param object $turnitintool The turnitintool object for this activity
 * @param var $userid The moodle user ID for the user viewing the report or the owner of the Turnitin class if the tutor
 * @param object $submission The submission data object from turnitintool_submissions
 * @param var $do The $do parameter from the query string to determine the current page view
 * @param object $loaderbar The loaderbar object passed by reference can be NULL if no loader bar is used
 */
function turnitintool_view_report($cm,$turnitintool,$userid,$submission,$do,&$loaderbar) {
    global $USER,$CFG;
    ...
    $tii=new turnitintool_commclass(turnitintool_getUID($thisuser),$thisuser->firstname,$thisuser->lastname,$thisuser->email,$utp,$loaderbar);
    $tii->startSession();
    $post->paperid=$submission->submission_objectid;
    
    turnitintool_redirect($tii->getReportLink($post,$thisuser));
    $tii->endSession();
    exit();
...
function turnitintool_percent_to_gradpos($percent,$left='36') {
    ...
    $style=' style="background: url(images/gradback.jpg) no-repeat '.$left.'px '.$pos.'px";';
    return $style;
}
 ...
 * @param object $cm The moodle course module object for this instance
 * @param object $turnitintool The turnitintool object for this activity
 * @param var $userid The Moodle User ID of the user deleting the submission
 * @param object $submission The submission data object from turnitintool_submissions
 */
...
function turnitintool_cansubmit($cm,$turnitintool,$user) { // Returns an array of users that can still submit if executed by a tutor or true if executed by a user
    ...
    if (has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id))) {
        ...
        foreach ($partsavailable as $partavailable) {
            if (is_array($studentusers)) {
				foreach ($studentusers as $studentuser) {
					if (!turnitintool_get_records_select('turnitintool_submissions','turnitintoolid='.$turnitintool->id.
															' AND submission_part='.$partavailable->id.' AND userid='.$studentuser->id) OR $turnitintool->reportgenspeed==1) {
						// If the student has not made a submission or reportgenspeed=1 ie reports can be overwritten add student to the array
        ...
        foreach ($partsavailable as $partavailable) {
            if (!turnitintool_get_records_select('turnitintool_submissions','turnitintoolid='.$turnitintool->id.' AND submission_part='.$partavailable->id.' AND userid='.$user->id)
				 OR $turnitintool->reportgenspeed==1) {
 ...
 */
 function turnitintool_view_submission_form($cm,$turnitintool,$submissionid=NULL) {
    ...
    $param_type=optional_param('type',0);
    $param_userid=optional_param('userid');
    $param_agreement=optional_param('agreement');
    ...
    if ($cansubmit AND $totalusers>0) {
		...
		$output.='var stringsArray = new Array("'.get_string('addsubmission','turnitintool').'","'
													...
													.get_string('anonenabled','turnitintool').'");';
		$output.='var submissionArray = new Array();';
		if (is_array($submissions)) {
			$i=0;
			foreach ($submissions as $submission) {
				$output.='submissionArray['.$i.'] = new Array("'.$submission->userid.'","'.$submission->submission_part.'","'.$submission->submission_title.'","'.$submission->submission_unanon.'");';
        ...
        unset($cells);
        $cells=array();
        $cells[0]=get_string('submissiontype', 'turnitintool').turnitintool_help_icon('submissiontype',get_string('submissiontype','turnitintool'),'turnitintool',true,false,'',true);
        ...
        if ($turnitintool->type==0) {
            ...
            $cells[1]='<select onchange="turnitintool_jumptopage(this.value)">';
            $cells[1].='<option label="Select Submission Type" value="'.$CFG->wwwroot.'/mod/turnitintool/view.php'.'?id='.
                        $cm->id.'&do=submissions"'.$selected[0].'>Select Submission Type</option>';
                        
            $cells[1].='<option label="-----------------" value="#">-----------------</option>';
            ...
            $param_type=1;
            $cells[1]='<input id="submissiontype" name="submissiontype" type="hidden" value="1" />'.get_string('fileupload','turnitintool');
        } else if ($turnitintool->type==2) {
            $param_type=2;
            $cells[1]='<input id="submissiontype" name="submissiontype" type="hidden" value="2" />'.get_string('textsubmission','turnitintool');
        } else {
            $param_type=3;
            $cells[1]='<input id="submissiontype" name="submissiontype" type="hidden" value="3" />'.get_string('webpage','turnitintool');
        }
    ...
    <form enctype="multipart/form-data" action="'.$CFG->wwwroot.'/mod/turnitintool/view.php'.'?id='.$cm->id.'&do=submissions&type='.$param_type.'" method="POST" name="submissionform">';
        ...
        $context = get_context_instance(CONTEXT_MODULE, $cm->id);
        if ($param_type!=0) {
            $submissiontitle=optional_param('submissiontitle','');
            $disableform=false;
            if (has_capability('mod/turnitintool:grade', get_context_instance(CONTEXT_MODULE, $cm->id))) {
                unset($cells);
                $cells=array();
                $cells[0]=get_string('studentsname', 'turnitintool').turnitintool_help_icon('studentsname',get_string('studentsname','turnitintool'),'turnitintool',true,false,'',true);

                if (count($cansubmit)>0) {
                    $cells[1]='<select name="userid" id="userid" onchange="updateSubForm(submissionArray,stringsArray,this.form,'.$turnitintool->reportgenspeed.')">';
                    ...
                    foreach ($cansubmit as $courseuser) {
                        ...
                        if (has_capability('mod/turnitintool:submit',$context, $courseuser->id, false)) {
							...
							$cells[1].='<option label="'.$courseuser->firstname.' '.$courseuser->lastname.
                    ...
                    if ($cells[1]=='<select id="userid" name="userid">'.'</select>') {
                        $cells[1]='<i>'.get_string('allsubmissionsmade','turnitintool').'</i>';
                        $disableform=true;
                    }
                } else {
                    $cells[1]='<i>'.get_string('noenrolledstudents','turnitintool').'</i>';
                }
                $table->data[]=$cells;
            } else {
				$cells[0]='';
				$cells[1]='<input id="userid" name="userid" type="hidden" value="'.$USER->id.'" />';
				$table->data[]=$cells;
			}
            ...
            if (!$disableform) {
                ...
                unset($cells);
                $cells=array();
                $cells[0]=get_string('submissiontitle', 'turnitintool').turnitintool_help_icon('submissiontitle',
                                     ...
                                     '',
                                     true);
                $cells[1]='<input type="text" name="submissiontitle" class="formwide" maxlength="200" value="'.$submissiontitle.'" />&nbsp;<span id="submissionnotice"></span>';
                $table->data[]=$cells;
                ...
                if (count($parts)>1) {
                    ...
                    unset($cells);
                    $cells=array();
                    $cells[0]=get_string('submissionpart', 'turnitintool').turnitintool_help_icon('submissionpart', get_string('submissionpart','turnitintool'),
                                         'turnitintool',true,false,'',true);
                    $cells[1]='<select name="submissionpart" class="formwide" onchange="updateSubForm(submissionArray,stringsArray,this.form,'.$turnitintool->reportgenspeed.')">';
                
                    $i=0;
                    foreach ($parts as $part) { // Do parts that have not yet been submitted to
                        $cells[1].='<option label="'.$part->partname.'" value="'.$part->id.'">'.$part->partname.'</option>';
                        $i++;
                    }
                    ...
                    unset($cells);
                    $cells=array();
                    $cells[0]=get_string('submissionpart', 'turnitintool').
                                turnitintool_help_icon('submissionpart',get_string('submissionpart','turnitintool'),'turnitintool',true,false,'',true);
    
                    foreach ($parts as $part) { // Do parts that have not yet been submitted to
                        $cells[1]=$part->partname.'<input type="hidden" name="submissionpart" value="'.$part->id.'" />';
                        break;
                    }
                    $table->data[]=$cells;
                }
                ...
                if ($param_type==1) {
                    unset($cells);
                    $cells=array();
                    $cells[0]=get_string('filetosubmit', 'turnitintool').turnitintool_help_icon('filetosubmit',get_string('filetosubmit','turnitintool'),'turnitintool',true,false,'',true);
                    $cells[1]='<input type="hidden" name="MAX_FILE_SIZE" value="'.$turnitintool->maxfilesize.'" />';
                    $cells[1].='<input type="file" name="submissionfile" size="55%" />';
                    $table->data[]=$cells;
                }
                
                if ($param_type==2) {
                    unset($cells);
                    $submissiontext=optional_param('submissiontext','');
                    
                    $cells=array();
                    $cells[0]=get_string('texttosubmit', 'turnitintool').turnitintool_help_icon('texttosubmit',get_string('texttosubmit','turnitintool'),'turnitintool',true,false,'',true);
                    $cells[1]='<textarea name="submissiontext" class="submissionText">'.$submissiontext.'</textarea>';
                    $table->data[]=$cells;
                }
                
                if ($param_type==3) {
                    unset($cells);
                    $submissionurl=optional_param('submissionurl','');
                    
                    $cells=array();
                    $cells[0]=get_string('urltosubmit', 'turnitintool').turnitintool_help_icon('urltosubmit',get_string('urltosubmit','turnitintool'),'turnitintool',true,false,'',true);
                    $cells[1]='<input type="text" name="submissionurl" class="formwide" value="'.$submissionurl.'" />';
                    $table->data[]=$cells;
                }
                ...
                if (has_capability('mod/turnitintool:grade', $context)) {
                    ...
                    $cells=array();
                    $cells[0]='';
                    $cells[1]='<input type="hidden" name="agreement" value="1" />';
                    $table->data[]=$cells;
                } else {
                    unset($cells);
                    $cells=array();
                    $cells[0]='<input type="checkbox" name="agreement" value="1"'.$checked.' />';
                    $cells[1]=$CFG->turnitin_agreement;
                    $table->data[]=$cells;
                }
                ...
                $cells=array();
                $cells[0]='&nbsp;';
                $cells[1]='<input name="submitbutton" type="submit" value="'.get_string('addsubmission', 'turnitintool').'" />';
                $table->data[]=$cells;
        ...
        $output.='
		<script language="javascript" type="text/javascript">updateSubForm(submissionArray,stringsArray,document.submissionform,'.$turnitintool->reportgenspeed.');</script>
        </form>
		';
        ...
        // Due date has not passed
            ...
            $context = get_context_instance(CONTEXT_MODULE, $cm->id);
			$users=get_users_by_capability($context,'mod/turnitintool:submit','u.id','','','','','',false);
...
function turnitintool_dofileupload($cm,$turnitintool,$userid,$post) {
    ...
    if (isset($post['submissiontitle'])) {
        $submissiontitle=$post['submissiontitle'];
    }
    ...
    if (turnitintool_duplicate_submission_title($turnitintool,$submissiontitle,$userid) AND $turnitintool->reportgenspeed!=1) {
    ...
    if (!$error AND isset($checksubmission->id) AND $turnitintool->reportgenspeed!=1) {
        ...
        turnitintool_print_error('alreadysubmitted','turnitintool','',NULL,__FILE__,__LINE__);
        exit();
    }
    ...
    if (substr_count(turnitintool_file_path($cm,$turnitintool,$userid),'/')>0) {
        $destination=turnitintool_file_path($cm,$turnitintool,$userid);
    } else {
        $destination=str_replace('/','\\',turnitintool_file_path($cm,$turnitintool,$userid));
    }
    ...
    if (!$error) {
		...
		if (!$resubmission) {
			if (!$insertid=turnitintool_insert_record('turnitintool_submissions',$submitobject)) {
...
function turnitintool_dotextsubmission($cm,$turnitintool,$userid,$post) {
    ...
    if (!$error) {
        ...
        $filedir=$CFG->dataroot.'/'.$destination;
...
function turnitintool_upload_submission($cm,$turnitintool,$submission) {
    global $CFG;
	 ...
	 $post->oid=(!is_null($submission->submission_objectid) AND !empty($submission->submission_objectid)) ? $submission->submission_objectid : '';
     
    $filepath=$CFG->dataroot.'/'.turnitintool_file_path($cm,$turnitintool,$submission->userid).'/'.$submission->submission_filename;
     
    if (substr_count(turnitintool_file_path($cm,$turnitintool,$submission->userid),'/')>0) {
        $destination=$filepath;
    } else {
        $destination=str_replace('/','\\',$filepath);
    }
    ...
    if ($tii->getRerror()) {
        $reason=($tii->getAPIunavailable()) ? get_string('apiunavailable','turnitintool') : $tii->getRmessage();
        turnitintool_print_error($reason." CODE: ".$tii->getRcode(),NULL,'',NULL,__FILE__,__LINE__);
        exit();
    }
...
function turnitintool_filetype_array($setup=true) {
    $output = array(
        ...
        3 => get_string('webpage','turnitintool')*/
        );
    if ($setup) {
        $output[NULL] = '--------------------';
...
function turnitintool_reset_course_form($course) {
    ...
    choose_from_menu($options,'reset_turnitintool','0');  echo '</p>';
}
/**
 * Function called by course/reset.php when resetting moodle course Moodle 1.8
 *
...
function turnitintool_duplicate_recycle($courseid,$action,$legacy=false) {
    ...
    foreach ($turnitintools as $turnitintool) {
        ...
        foreach ($parts as $part) {
            ...
            $partsarray[$courseid][$turnitintool->id][$part->id]['uid']=turnitintool_getUID($owner);
            $partsarray[$courseid][$turnitintool->id][$part->id]['uem']=$owner->email;
            $partsarray[$courseid][$turnitintool->id][$part->id]['ufn']=$owner->firstname;
            $partsarray[$courseid][$turnitintool->id][$part->id]['uln']=$owner->lastname;
    ...
    foreach ($partsarray as $classid => $v1) {
        foreach ($v1 as $assignid => $v2) {
            foreach ($v2 as $partid => $data) {
                ...
                $tii = new turnitintool_commclass($data['uid'],$data['ufn'],$data['uln'],$data['uem'],2,$loaderbar);
                $tii->startSession();
...
function turnitintool_buildgrades($turnitintool,$thisuser) {
    if ($submissions=turnitintool_get_records_select('turnitintool_submissions','turnitintoolid='.$turnitintool->id.' AND userid='.$thisuser->id.' AND submission_unanon=1')) {
        ...
        $overallgrade=number_format($gradearray->grades[$thisuser->id],1);
        if ($overallgrade=='-') {
            $overallgrade=NULL;
        }
...
function turnitintool_help_icon($page, $title, $module='moodle', $image=true, $linktext=false, $text='', $return=false, $imagetext='') {
    ...
    if (is_callable(array($OUTPUT,'help_icon'))) {
        $icon=moodle_help_icon::make($page, $title, $module, $linktext);
        $icon->image=$image;
        if (!$return) {
            echo $OUTPUT->help_icon($icon);
        } else {
            return $OUTPUT->help_icon($icon);
        }
    } else {
...
/**
 * Abstracted version of print_table() / table() to work with Moodle 1.8 through 2.0
 *
...
function turnitintool_print_table($table, $return=false) {
    global $OUTPUT;
    if (is_callable(array($OUTPUT,'table'))) {
 ...
 */
function turnitintool_print_error($input,$module=NULL,$link='',$param=NULL,$file=__FILE__,$line=__LINE__) {
    global $CFG;
    if (is_null($module)) {
        $message=$input;
    } else {
