<?PHP  // $Id: lib.php,v 1.13.10.7 2009/03/31 22:54:59 diml Exp $
...
function scheduler_delete_instance($id) {
    ...
    include $CFG->dirroot."/mod/scheduler/locallib.php";
...
function scheduler_delete_userdata($data, $showfeedback = true) {
    ...
    if (!empty($data->reset_appointments) || !empty($data->reset_slots)) {
        if (execute_sql($sql_appointments, false) and $showfeedback) {
            notify($strreset.': '.get_string('appointments','scheduler'), 'notifysuccess');
        }
    }
    if (!empty($data->reset_slots)) {
        if (execute_sql($sql_slots, false) and $showfeedback) {
...
/**
 * get list of teachers who can be assigned slots
 * @param object context module context
 * @return array of user objects for possible teachers
 */
function scheduler_getteachers($context) {
    global $COURSE, $USER;
    $coursecontext = get_context_instance(CONTEXT_COURSE, $COURSE->id);

    // all the current user to be selected if they have the legacy role but not the attend capability
    if (has_capability('moodle/legacy:teacher', $coursecontext) && !has_capability('mod/scheduler:attend', $context)){
        $teachers = get_record('user', 'id', $USER->id, 'lastname', 'id,lastname,firstname,email,picture');
    } else {
        $teachers = array();
    }

    // the remainder have the required capability
    $teachers = array_merge($teachers, get_users_by_capability ($context, 'mod/scheduler:attend', 'u.id,lastname,firstname,email,picture', 'lastname'));
    return $teachers;
}
