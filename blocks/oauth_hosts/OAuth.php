...
class OAuthServer {
    ...
    function get_consumer(&$request) {
        ...
        if (!$consumer) {
            throw new OAuthException("Invalid consumer ($consumer_key)");
        }
        return $consumer;
    }
    ...
    function get_token(&$request, $consumer, $token_type = "access") {
        ...
        if (!$token) {
            throw new OAuthException("Invalid$token_typetoken:$token_field");
        }
        return $token;
    }
    ...
    function check_signature(&$request, $consumer, $token) {
        ...
        if (!$valid_sig) {
            ...
            if ($OAuth_last_computed_signature) {
                $ex_text = $ex_text . "ours=$OAuth_last_computed_signatureyours=$signature";
            }
